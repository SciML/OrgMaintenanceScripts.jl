...
"""
function create_compat_pr(
        repo_path::String, bumped_packages::Vector{String}, fork_user::String)
    cd(repo_path) do
        # Get repo info
...
function create_compat_pr_multi(
        repo_path::String, bumped_info::Dict{String, Vector{String}}, fork_user::String)
    cd(repo_path) do
        # Get repo info
        remote_url = strip(read(`git remote get-url origin`, String))
        m = match(r"github\.com[/:]([^/]+)/([^/]+?)(?:\.git)?$", remote_url)
...
function bump_compat_org_repositories(org::String = "SciML";
    ...
    if isempty(log_file)
        ...
        mkpath(log_dir)
        log_file = joinpath(
            log_dir, "compat_bump_$(org)_$(Dates.format(now(), "yyyy-mm-dd_HHMMSS")).log")
    end
    ...
    open(log_file, "w") do log_io
        ...
        println(
            log_io, "# Target package: $(isnothing(package_name) ? "all" : package_name)")
