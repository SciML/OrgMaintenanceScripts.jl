...
"""
function find_version_checks_in_file(filepath::String; min_version::VersionNumber=JULIA_LTS)
    if !isfile(filepath)
...
"""
function write_version_checks_to_script(checks::Vector{VersionCheck}, output_file::String="fix_version_checks.jl")
...
"""
function write_org_version_checks_to_script(org_results::Dict{String, Vector{VersionCheck}}, output_file::String="fix_org_version_checks.jl")
...
function fix_version_checks_parallel(checks::Vector{VersionCheck}, n_processes::Int=4;
    ...
    @everywhere function process_repo_fixes(repo_path::String, repo_checks::Vector{VersionCheck}, 
        ...
        try
            ...
            open(fix_script, "w") do io
                ...
                for check in repo_checks
                    println(io, "# - $(check.file):$(check.line_number) - version $(check.version)")
                end
            end
    ...
    results = pmap(pairs(checks_by_repo)) do (repo_path, repo_checks)
        process_repo_fixes(repo_path, repo_checks, github_token, base_branch, pr_title_prefix)
    end
...
function fix_org_version_checks_parallel(org::String, n_processes::Int=4;
    ...
    org_checks = find_version_checks_in_org(org; min_version, auth_token=github_token, work_dir)
    ...
    # Fix in parallel
    results = fix_version_checks_parallel(all_checks, n_processes; github_token, base_branch)
