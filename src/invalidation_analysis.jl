...
function analyze_major_invalidators(invalidation_data::Dict)
    ...
    for inv in invalidations
        ...
        package_stats[pkg]["max_children"] = max(
            package_stats[pkg]["max_children"], inv["children_count"])
...
function generate_invalidation_report(repo_path::String, test_script::String = "")
    ...
    try
        ...
        if total_invalidations > 0
            ...
            push!(recommendations,
                "Profile with `@profile` to identify performance bottlenecks")
        else
            push!(recommendations,
                "Great job! No invalidations detected. Your package is well-optimized.")
        end
...
"""
function analyze_repo_invalidations(
        ...
        try
            ...
            if !isempty(output_dir)
                ...
                repo_report_file = joinpath(
                    output_dir, "$(basename(repo_name))_invalidations.json")
...
"""
function generate_org_summary_report(
        org::String, results::Dict{String, InvalidationReport}, output_dir::String)
    if isempty(output_dir)
        output_dir = tempdir()
    end
    ...
    open(summary_file, "w") do io
        ...
        println(io,
            "- **Average Invalidations per Repo**: $(round(avg_invalidations, digits=1))")
        ...
        elseif avg_invalidations < 20
            ...
            println(
                io, "❌ High invalidation levels detected. Immediate attention recommended.")
        end
        ...
        println(io,
            "3. Consider creating organization-wide coding guidelines for type stability")
        println(io, "4. Set up CI checks for invalidation regression testing")
        println(io)

        println(io, "## Detailed Results")
        for (repo, report) in sort(
            collect(results), by = x -> x[2].total_invalidations, rev = true)
            println(io, "### $repo")
